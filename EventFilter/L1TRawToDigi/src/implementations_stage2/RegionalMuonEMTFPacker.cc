...
namespace l1t {
   namespace stage2 {
      class RegionalMuonEMTFPacker : public Packer {
         public:
            virtual Blocks pack(const edm::Event&, const PackerTokens*) override;
         private:
            typedef std::map<unsigned int, std::vector<uint32_t>> PayloadMap;
	// Do we need to add an EMTFMuonCandBxCollection? - AWB 11.01.16
...
namespace l1t {
   namespace stage2 {
      Blocks
      RegionalMuonEMTFPacker::pack(const edm::Event& event, const PackerTokens* toks)
      {
      ...
      void
      RegionalMuonEMTFPacker::packTF(const edm::Event& event, const edm::EDGetTokenT<RegionalMuonCandBxCollection>& tfToken, Blocks &blocks, const std::vector<unsigned int>& links)
      {
	 ...
	 // The following seems specific to GMT ... what to do for EMTF? - AWB 11.01.15
...
DEFINE_L1T_PACKER(l1t::stage2::RegionalMuonEMTFPacker);
